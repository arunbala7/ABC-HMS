...
SET isInMed_issued = EXISTS (SELECT * From medicine_issued Where patient_id = p_id AND medicine_id = m_id );
...
ELSE
	INSERT INTO  medicine_issued (medicine_id, patient_id, quantity_issued) VALUES ( m_id , p_id, reqested);
	UPDATE medicine_master SET quantity_available = quantity_available - reqested WHERE medicine_id = m_id;
END IF;
END$$
DELIMITER ;
